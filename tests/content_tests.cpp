...
void wavesSpawnNestedWispsThatCloseIn() {
    ...
    // The authored emission volume survived the load: an unrecognised shape string degrades to
    // Point without a warning, so a content typo here is invisible to every other check.
...
void caughtWispsBurstAndCleanUp() {
    ...
    // The burst kept its authored radial shell through the silent shape fallback.
...
void titleAttractsIntoTheArena() {
    ...
    // The title motes kept their authored box volume through the silent shape fallback.
