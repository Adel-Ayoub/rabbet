...
const std::string& builtinWaterVertexSource() {
    // A unit XZ quad scaled and placed by uModel; every wave lives in the fragment shader, so the
    // mesh stays four vertices no matter the extent.
    static const std::string source = R"(#version 410 core
layout(location = 0) in vec3 aPosition;
...
const std::string& builtinWaterFragmentSource() {
    // Procedural sine-octave waves with analytic slopes (no normal-map assets), a fresnel blend
    // into the scene's own skybox cubemap, Blinn-Phong sun streaks, and the shared uHdrOutput
    // seam. The quad edge fades out so a surface never ends in a hard line.
    static const std::string source = R"(#version 410 core
in vec3 vWorldPos;
in vec2 vLocal;
out vec4 FragColor;
